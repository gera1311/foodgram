...
class CreateUpdateDeleteRecipeSerializer(serializers.ModelSerializer):
    ...
    def update(self, instance, validated_data):
        ...
        if image:
            instance.image = image  # Если новое изображение передано
        else:
            # Если изображение не передано, оставляем старое
            validated_data['image'] = instance.image

        if ingredients_data:
            RecipeIngredient.objects.filter(recipe=instance).exclude(
                ingredient_id__in=[ing['id'] for ing in ingredients_data]
            ).delete()
